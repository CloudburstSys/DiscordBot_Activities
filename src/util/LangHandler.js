            ...
            if (content == undefined) {
                ...
                if (path.endsWith("/" + pathPart)) {
                    if (typeof content != "string") {
                        if (content["_"] == undefined) throw new TypeError("The string path '" + path + "' returns an object, but doesn't have a '_' (header) field and thus cannot be treated as a string.");
                        content = content["_"];
                    } else {
                        // `content` is already what we want it to be
                    }
                }
            }
        });
